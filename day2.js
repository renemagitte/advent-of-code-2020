...
function solvePuzzle(puzzleInput) {
    ...
    puzzleInput.map(passwordCombination => {
        const ruleAsString = passwordCombination[0];
        const password = passwordCombination[1];

        const ruleSplit = ruleAsString.split(" ");
        const requiredChar = ruleSplit[1]
        const minOccurance = ruleSplit[0].split("-")[0];
        const maxOccurance = ruleSplit[0].split("-")[1];

        const countOccurancesOfRequriredChar = (password.split(requiredChar).length - 1);

        const withinMinLimit = countOccurancesOfRequriredChar >= minOccurance;
        const withinMaxLimit = countOccurancesOfRequriredChar <= maxOccurance;
